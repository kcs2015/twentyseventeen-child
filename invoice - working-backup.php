...
<tr id="pmt-sch-row">
        ...
        <table class="invoice-large-info-table">
            ...
            if (!empty($initial_payment_arr)): ?>
                <tr class="bg-color-secondary">
                    ...
                    <td>$<?php echo(number_format ($initial_payment_arr[1])); ?>  </td>
                </tr>
           <?php endif;
            ...
            // there are meta
                       foreach( $rsv_info_payment_arr as $payment_num=> $payment_amt ):
                           ...
                           <tr class="bg-color-secondary">
                               ...
                               <td>$ <?php echo(number_format ($rsv_payment_arr[1])); ?>  </td>
                           </tr>
                     <?php   endforeach;
                    // get remaining payment amounts
            // Get current balance
            $num_payments_due  = get_post_meta( $curr_order->ID, 'num_payments_due', TRUE);
            ...
            $invoice_due_date_arr= array("6/30/2017","7/31/2017","8/30/2017","9/31/2017");
            ...
            while ($invoice_start_num < count($invoice_due_date_arr)): ?>
                <tr><td><?php echo($invoice_due_date_arr[$invoice_start_num]); ?></td>
                    <td>$<?php echo (number_format ($remaining_payments)); ?></td>
                </tr>
    ...
    <td>
        <table id="travel-details-tbl" class="invoice-large-info-table float-right">
            <thead class="bg-color-primary">
            ...
            <tr><td>Adult Guests:</td>
                ...
                <tr><td>Resort + <?php echo($air_service_requested_html); ?>:</td>
                <td>$ <?php echo(number_format ($rsv_info_invoice_total)) ?></td>
            </tr>
                </tbody>
        </table>
        ...
        <tr id="bal-due-row">
            <td>
                ...
                <td>$<?php echo (number_format ( $total_remaining_balance)); ?></td>
            </tr>
        </table>
                 <div class="info-table-sub-text">(Price Includes Taxes and Fees)</div>
            </td>
            <td>
                <table class="due-date-info-table float-right">
                <tbody>
                    <tr><td>Total Due:</td><td>$<?php

                            echo(number_format ($current_payment_due)); ?></td></tr>
        ...
        <table id="total-package-breakdown-tbl" class="invoice-large-info-table">
           ...
           <tr><td>Child Guests:</td>
            ...
            <tr><td>Resort Package Total:</td>
                <td>$ <?php echo(number_format ($resort_rsv_total)) ?></td>
            </tr>
            <tr><td><?php echo($air_service_requested_html); ?> Total (For All Guests):</td>
                <td>$
                    <?php if($rsv_info_air_service_requested == "flight"):
                        echo(number_format ($flight_rsv_total));

                        elseif($rsv_info_air_service_requested == "transfers"):
                            echo(number_format ($airport_transfers_rsv_total)); ?>
                    <?php endif; ?>
                </td>
            </tr>
                <tr><td>Resort + <?php echo($air_service_requested_html); ?>
                        Totals (For All Guests):</td>
                <td>$ <?php echo(number_format ($rsv_info_invoice_total)) ?></td>
            </tr>
     ...
     <td>
        <table id="per-guest-breakdown" class="invoice-large-info-table float-right">
            ...
            <tr><td>Resort Rate Per Guest Per Night</td>
                <td>$<?php echo (number_format ($rsv_info_resort_rate_per_guest_per_night)); ?>
                </td>
            </tr><tr><td>Number of Nights</td>
                <td><?php echo ($resort_num_nights); ?></td>
            </tr>
            <tr><td>Resort Total Per Guest</td>
                <td>$<?php echo (number_format ($rsv_info_resort_total_per_guest)); ?></td>
            </tr><tr><td><?php echo($air_service_requested_html); ?> Total Per Guest</td>
                <td>$<?php echo (number_format ($rsv_info_air_service_rate_per_guest)); ?></td>
            </tr>
        <tr><td>Resort + <?php echo($air_service_requested_html); ?> Total Per Guest</td>
                <td>$<?php
                    // Get flight rate
                    ...
                    echo (number_format ($invoice_total_per_guest)); ?></td>
            </tr>
        </table>
    </td>
    </tr>
